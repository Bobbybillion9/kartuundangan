...
- **`templates/`** — one subfolder per visual theme, each a self-contained `index.html` + `style.css` (own inline script for demo/preview behavior, overridden at runtime by `assets/render-undangan.js`). Organized by category, currently only `elegan-klasik/` (`ivory-gold`, `sage-rose`, `emerald-dusk`). More categories can be added later.
  - **Photo slots must ship empty.** Every `<img data-rv="img">` inside a photo slot carries no `src` and starts `display:none`, so the theme's own designed empty state (icon + "Foto Mempelai Pria" caption) is what shows until `setSlotFoto()` in `render-undangan.js` swaps in the couple's real photo. Do **not** bake demo photos into a theme's slots: sage-rose and ivory-gold used to, and since `templates/pratinjau.html` renders the raw template, every prospective customer previewing those themes saw the stock placeholders as if they were the design — which included a dog captioned "Foto Mempelai Pria" and a tennis court captioned "Foto Mempelai Wanita". The only images a theme folder should keep are `assets/thumbnail.jpg` (referenced by `THEME_TEMPLATES`) and anything the landing page's hero corridor explicitly lists. `templates/pratinjau.html` renders any one theme standalone (used by "Preview" links and the dashboard's live preview tab).
