...
- **`templates/`** — one subfolder per visual theme, each a self-contained `index.html` + `style.css` (own inline script for demo/preview behavior, overridden at runtime by `assets/render-undangan.js`). Organized by category, currently only `elegan-klasik/` (`ivory-gold`, `sage-rose`, `emerald-dusk`). A `budaya/jawa-mandala-emas` theme existed as an experiment but was removed — it was never registered in `THEME_TEMPLATES`, so it never shipped. More categories can be added later.
